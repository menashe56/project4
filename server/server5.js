...
function padSequences(sequences, maxLen, padding = 'pre') {
...
loadCSV(imdbDatasetPath, (result) => {
  ...
  model.fit(ReviewsTensor, labelsTensor, {
    ...
    callbacks: {
      onEpochEnd: (epoch, logs) => {
        console.log(`Epoch ${epoch + 1}/${100} - loss: ${logs.loss.toFixed(4)} - accuracy: ${logs.acc.toFixed(4)}`);
      }
    }
  })
    .then(info => {
  ...
  const newReview = "the movie is great"; // Replace with your own review
