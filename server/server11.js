...
loadCSV(imdbDatasetPath, async (result) => {
  ...
  model.fit(ReviewsTensor, labelsTensor, {
    ...
    .then(async (info) => {
      ...
      fs.writeFileSync(modelArchitecturePath, JSON.stringify(model.toJSON(), null, 2));

      console.log('Model architecture saved successfully:', modelArchitecturePath);
      
      const vocabularyPath = './model/vocabulary.json';
      fs.writeFileSync(vocabularyPath, JSON.stringify(vocabulary, null, 2));

      console.log('Model vocabulary saved successfully:', vocabularyPath);
